...
double get_one_coeff ( double coeff_func );
coeff get_all_coeff ( coeff coeff_func );
...
int main()
{
    ...
    coeff_func = get_all_coeff ( coeff_func );
...
double get_one_coeff ( coeff coeff_func )
{
    int c, i, indicator = 0;
    i = 0;
    char s[100];
...
coeff get_all_coeff ( coeff coeff_func )
{
    coeff_func.a = get_one_coeff ( coeff_func );
    printf ( "A = %g\n", coeff_func.a );

    coeff_func.b = get_one_coeff ( coeff_func );
    printf ( "B = %g\n", coeff_func.b );

    coeff_func.c = get_one_coeff ( coeff_func );
    printf ( "C = %g\n", coeff_func.c );
